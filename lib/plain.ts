...
export const plainSummary: Record<string, { fr: string; en: string }> = {
  ...
  "cantelcox": {
    fr: "Le système qui gère les abonnés d'un opérateur mobile : changer de fournisseur en gardant son numéro, activer une ligne, payer sa facture. Je l'ai réorganisé pour que les étapes se passent le message plutôt que de s'attendre les unes les autres. Puis je l'ai mis sous charge cinq fois de suite : la commande est confirmée 2,4 fois plus vite, et la part des commandes qui vont jusqu'au bout est passée de 47 % à 100 %. Le dernier essai n'a rien changé — je l'ai gardé et écrit quand même.",
    en: "The system that manages a mobile carrier's subscribers: switching provider while keeping your number, activating a line, paying a bill. I reorganized it so the steps pass messages to each other instead of waiting on one another. Then I put it under load five times over: orders are confirmed 2.4 times faster, and the share of orders that make it all the way through went from 47% to 100%. The last attempt changed nothing — I kept it and wrote it up anyway.",
  },
  "financej": {
