...
const cantelcoxFr: CaseStudyV2 = {
  modes: ["recruiter", "engineering"],
  identity: {
    valueProp: "Un BSS télécom passé de microservices synchrones à une architecture événementielle — et surtout une campagne de mesure qui dit où est le goulot au lieu d'annoncer une victoire.",
    ...
    period: "2026",
    team: "Équipe · 48 des 97 commits de la branche de remise sont les miens",
    role: "Architecture & développement · contributeur principal",
  ...
  sections: [
    { id: "resume", title: "Résumé", modes: ["recruiter"],
      body: "CanTelcoX est le système commercial d'un opérateur mobile : portabilité de numéro, activation de ligne, facturation, MFA, anti-fraude. En Phase 2, je fais passer le système de microservices synchrones à une architecture orientée événements — 6 services, saga chorégraphiée avec compensation, Outbox transactionnel, DLQ, traçage distribué. Le livrable dont je suis le plus fier n'est pas la liste de patrons : c'est la campagne de charge, qui montre un gain réel de 43 % sur le P95 d'acquittement et prouve, chiffres à l'appui, que la convergence de saga ne s'est pas améliorée — et pourquoi." },
    { id: "contexte", title: "Contexte", modes: ["recruiter", "engineering"],
      body: "Cours d'architecture logicielle (LOG430) à l'ÉTS. Les labos 2 à 9 isolent chacun un patron sur la même application jouet ; le projet de session est le seul endroit où ces patrons doivent tenir ensemble, sur un domaine métier réel, avec des intégrations externes qu'on ne contrôle pas et des mesures qu'on doit concevoir soi-même." },
    { id: "role", title: "Mon rôle & l'équipe", modes: ["recruiter", "engineering"],
      body: "Projet d'équipe, suivi sur GitHub Classroom. Honnêtement : le mérite est collectif, mais j'ai signé 48 des 97 commits de la branche de remise — sur le châssis de messagerie (enveloppe, outbox, relais, consommateur, DLQ), les couches anticorruption et la campagne de mesure." },
    { id: "architecture", title: "Architecture", modes: ["engineering"], kind: "flow",
      body: "Six services FastAPI en hexagonal (domaine sans dépendance technique), une base logique par service, RabbitMQ comme bus, KrakenD comme unique point d'entrée REST et NGINX en répartiteur devant les répliques.",
      points: [
        "svc-identity (clients, MFA, JWT) · svc-subscription (lignes, activation, portabilité) · svc-billing (paiement, usage, facturation)",
        "svc-catalog (lecture, cache Redis) · svc-compliance (anti-fraude, journal d'audit) · svc-ordering (commandes, saga)",
        "Saga chorégraphiée avec compensation — pas d'orchestrateur central",
        "Outbox transactionnel : l'événement est écrit dans la même transaction que l'état métier, un relais le publie ensuite",
        "DLQ pour les messages non traitables, Jaeger (OTLP) pour le traçage, Grafana sur les 4 Golden Signals",
      ] },
    { id: "integrations", title: "Intégrations externes réelles", modes: ["engineering"],
      ...
      points: [
        "free5GC — cœur réseau 5G réel, provisionnement HLR/UDM à l'activation de ligne",
        "PortabilityHub — hub de portabilité en Java, callbacks par webhook via un proxy dédié",
        "Écart de spécification relevé et corrigé : le Hub expose 5 statuts, la spécification du cours en annonçait 3",
      ] },
    { id: "mesures", title: "Ce que la mesure a montré", modes: ["recruiter", "engineering"], kind: "metrics",
      body: "Campagne k6, montée 10 → 50 → 100 VU sur 3 minutes, deux configurations comparées (intervalle de balayage outbox 2 s → 0,5 s, prefetch 1 → 4, svc-ordering 1 → 2 répliques). Deux métriques distinctes, et la distinction est tout le sujet : l'acquittement du POST /v1/orders, et la convergence de la saga jusqu'à COMPLETED. Un acquittement rapide ne prouve rien si la chaîne derrière met 30 s." },
    { id: "goulot", title: "Le goulot s'est déplacé — et il est chiffré", modes: ["engineering"],
      body: "Baisser l'intervalle de balayage supprime un plancher structurel de latence (trois sauts de saga coûtaient au pire 6 s à eux seuls). Mais la convergence ne s'améliore pas sous 100 VU, et l'explication est mesurée, pas supposée.",
      points: [
        "3 960 messages en file sur subscription.paiement_autorise, avec un seul consommateur",
        "2 228 paiements autorisés contre 788 lignes provisionnées — un déséquilibre de 2,8:1",
        "Cause : l'appel HLR est synchrone dans la transaction d'activation",
        "Conclusion assumée : accélérer l'acceptation sans accélérer le traitement aval ne fait que déplacer la file",
      ] },
    { id: "defauts", title: "Deux défauts trouvés par la donnée", modes: ["engineering"],
      points: [
        "1 467 événements dupliqués : deux relais outbox lisaient le même lot. Corrigé par with_for_update(skip_locked=True) et commit en fin de lot — 0 doublon après correctif, vérifié.",
        "Un « health check » menteur : le service répondait 200 alors que son thread consommateur était mort (except Exception: break, jamais relancé). La sonde teste désormais l'état des consommateurs — vérifié par injection de panne : 503 pendant la coupure de RabbitMQ, retour à 200 sans redémarrer aucun service.",
      ] },
    { id: "discipline", title: "Discipline architecturale automatisée", modes: ["engineering"],
      body: "L'isolation du domaine n'est pas une promesse dans un document : elle casse la CI si un import technique entre dans domain/ (import-linter). Documentation Arc42 + vue 4+1 + Event Storming, et 7 ADR pour les décisions structurantes." },
    { id: "limites", title: "Limites", modes: ["recruiter", "engineering"], kind: "limits" },
  ],
  metrics: [
    { label: "P95 d'acquittement (référence → optimisée)", value: "952 → 539 ms", evidence: "measured", note: "−43 %, campagne k6 du 2026-08-06" },
    { label: "débit HTTP", value: "80 → 98,7 req/s", evidence: "measured", note: "+23 %" },
    { label: "taux d'erreur HTTP sous 100 VU", value: "0 %", evidence: "measured" },
    { label: "événements dupliqués (avant → après correctif)", value: "1 467 → 0", evidence: "measured" },
    { label: "microservices + passerelle + répartiteur", value: "6", evidence: "implemented" },
    { label: "ADR · Arc42 · 4+1 · Event Storming", value: "7 ADR", evidence: "designed" },
    { label: "bascule NGINX, réplique tuée", value: "20/20 en HTTP 200", evidence: "measured" },
    { label: "mes commits sur la branche de remise", value: "48 / 97", evidence: "implemented" },
  ],
  decisions: [
    {
      id: "saga",
      title: "Saga chorégraphiée plutôt qu'orchestrée",
      context: "La Phase 1 coordonnait l'activation de ligne par appels REST synchrones : un service lent bloquait toute la chaîne.",
      decision: "Chaque service réagit aux événements des autres et publie sa compensation en cas d'échec, sans orchestrateur central.",
      rationale: "Découplage temporel : la commande est acceptée en 202 même si le HLR est lent, et aucun service n'a besoin de connaître le graphe complet.",
      tradeoff: "Le flux n'est plus lisible en un seul endroit — d'où le traçage distribué et le catalogue d'événements documenté, sans quoi le débogage devient impraticable.",
    },
    {
      id: "outbox",
      title: "Outbox transactionnel + verrouillage skip_locked",
      ...
      decision: "L'événement est écrit dans la table outbox dans la même transaction que l'état métier ; un relais le publie ensuite, en lisant son lot avec with_for_update(skip_locked=True) et en committant en fin de lot.",
      ...
      tradeoff: "Livraison au moins une fois : les consommateurs doivent être idempotents. La mesure a d'ailleurs révélé 1 467 doublons avant le verrouillage.",
    },
    {
      id: "acl",
      ...
      decision: "Toute traduction se fait dans des adapters d'infrastructure ; le domaine ne connaît que ses propres concepts.",
      rationale: "Un changement chez le Hub — comme les 5 statuts réels au lieu des 3 annoncés — se corrige dans un adapter, pas dans les règles métier.",
    ...
    },
    {
      ...
      decision: "import-linter interdit tout import technique dans domain/, et la CI échoue si la règle est franchie.",
      rationale: "La contrainte devient structurelle plutôt que déclarative — c'est la seule forme de discipline qui survit à la pression.",
      tradeoff: "Quelques détours d'implémentation (ports explicites) là où un import direct aurait été plus court.",
    },
  ],
  limits: [
    "Projet académique : échelle de labo, pas de charge de production réelle ni d'utilisateurs.",
    "La cible du cahier (≥ 1 000 ops/s) n'est pas atteinte : 98,7 req/s HTTP, soit ~1,5 saga complète par seconde sous 100 VU. L'écart est localisé et chiffré, pas expliqué après coup.",
    "Le seuil de convergence de saga (95 % sous 30 s) n'est pas atteint sous 100 VU : la convergence est fonctionnelle mais non dimensionnée pour cette charge avec un HLR synchrone.",
    "Projet d'équipe : 48 des 97 commits de la branche de remise sont les miens.",
    "Dépôt privé (GitHub Classroom) — le code ne peut pas être publié ; les chiffres ci-dessus viennent du rapport de mesures de la campagne du 2026-08-06.",
  ],
  links: [],
};
...
const cantelcoxEn: CaseStudyV2 = {
  modes: ["recruiter", "engineering"],
  identity: {
    valueProp: "A telecom BSS moved from synchronous microservices to event-driven architecture — and, above all, a measurement campaign that says where the bottleneck is instead of announcing a win.",
    ...
    period: "2026",
    team: "Team · 48 of the 97 commits on the submission branch are mine",
    role: "Architecture & development · lead contributor",
    stack: ["Python 3.12 · FastAPI", "RabbitMQ", "MySQL · Flyway", "Redis", "KrakenD · NGINX", "Prometheus / Grafana", "Jaeger (OTLP)", "k6", "Docker Compose", "free5GC", "import-linter"],
    updated: "2026-09-09",
  },
  sections: [
    { id: "resume", title: "Summary", modes: ["recruiter"],
      body: "CanTelcoX is a mobile carrier's business system: number portability, line activation, billing, MFA, fraud screening. In Phase 2 I move it from synchronous microservices to an event-driven architecture — 6 services, a choreographed saga with compensation, a transactional Outbox, DLQ, distributed tracing. The deliverable I'm proudest of isn't the pattern checklist: it's the load campaign, which shows a real 43% gain on acknowledgement P95 and proves, with numbers, that saga convergence did not improve — and why." },
    { id: "contexte", title: "Context", modes: ["recruiter", "engineering"],
      body: "ÉTS's software-architecture course (LOG430). Labs 2 through 9 each isolate one pattern on the same toy application; the term project is the only place where those patterns have to hold together, on a real business domain, with external integrations you don't control and measurements you have to design yourself." },
    { id: "role", title: "My role & the team", modes: ["recruiter", "engineering"],
      body: "A team project, tracked on GitHub Classroom. Honestly: the credit is collective, but I signed 48 of the 97 commits on the submission branch — on the messaging chassis (envelope, outbox, relay, consumer, DLQ), the anti-corruption layers and the measurement campaign." },
    { id: "architecture", title: "Architecture", modes: ["engineering"], kind: "flow",
      body: "Six FastAPI services in a hexagonal layout (a domain with no technical dependencies), one logical database per service, RabbitMQ as the bus, KrakenD as the single REST entry point and NGINX load-balancing the replicas.",
      points: [
        "svc-identity (customers, MFA, JWT) · svc-subscription (lines, activation, portability) · svc-billing (payment, usage, invoicing)",
        "svc-catalog (read side, Redis cache) · svc-compliance (fraud screening, audit log) · svc-ordering (orders, saga)",
        "Choreographed saga with compensation — no central orchestrator",
        "Transactional Outbox: the event is written in the same transaction as the business state, a relay publishes it afterwards",
        "DLQ for unprocessable messages, Jaeger (OTLP) for tracing, Grafana on the 4 Golden Signals",
      ] },
    { id: "integrations", title: "Real external integrations", modes: ["engineering"],
      ...
      points: [
        "free5GC — a real 5G core, HLR/UDM provisioning on line activation",
        "PortabilityHub — a Java portability hub, webhook callbacks through a dedicated proxy",
        "Spec discrepancy found and corrected: the Hub exposes 5 statuses where the course spec announced 3",
      ] },
    { id: "mesures", title: "What the measurement showed", modes: ["recruiter", "engineering"], kind: "metrics",
      body: "A k6 campaign ramping 10 → 50 → 100 VUs over 3 minutes, two configurations compared (outbox poll interval 2s → 0.5s, prefetch 1 → 4, svc-ordering 1 → 2 replicas). Two distinct metrics, and the distinction is the whole point: the acknowledgement of POST /v1/orders, and the saga's convergence to COMPLETED. A fast acknowledgement proves nothing if the chain behind it takes 30 seconds." },
    { id: "goulot", title: "The bottleneck moved — and it's quantified", modes: ["engineering"],
      body: "Lowering the poll interval removes a structural latency floor (three saga hops cost up to 6 seconds on their own). But convergence does not improve under 100 VUs, and the explanation is measured, not assumed.",
      points: [
        "3,960 messages queued on subscription.paiement_autorise, with a single consumer",
        "2,228 payments authorized against 788 lines provisioned — a 2.8:1 imbalance",
        "Cause: the HLR call is synchronous inside the activation transaction",
        "Owned conclusion: speeding up acceptance without speeding up downstream processing only relocates the queue",
      ] },
    { id: "defauts", title: "Two defects found by data", modes: ["engineering"],
      points: [
        "1,467 duplicated events: two outbox relays were reading the same batch. Fixed with with_for_update(skip_locked=True) and an end-of-batch commit — 0 duplicates after the fix, verified.",
        "A lying health check: the service returned 200 while its consumer thread was dead (except Exception: break, never restarted). The probe now tests consumer state — verified by fault injection: 503 during the RabbitMQ outage, back to 200 without restarting any service.",
      ] },
    { id: "discipline", title: "Architectural discipline, automated", modes: ["engineering"],
      body: "Domain isolation isn't a promise in a document: it breaks CI if a technical import reaches domain/ (import-linter). Arc42 documentation + the 4+1 views + Event Storming, and 7 ADRs for the structuring decisions." },
    { id: "limites", title: "Limits", modes: ["recruiter", "engineering"], kind: "limits" },
  ],
  metrics: [
    { label: "acknowledgement P95 (baseline → tuned)", value: "952 → 539 ms", evidence: "measured", note: "−43%, k6 campaign of 2026-08-06" },
    { label: "HTTP throughput", value: "80 → 98.7 req/s", evidence: "measured", note: "+23%" },
    { label: "HTTP error rate under 100 VUs", value: "0%", evidence: "measured" },
    { label: "duplicated events (before → after fix)", value: "1,467 → 0", evidence: "measured" },
    { label: "microservices + gateway + load balancer", value: "6", evidence: "implemented" },
    { label: "ADRs · Arc42 · 4+1 · Event Storming", value: "7 ADRs", evidence: "designed" },
    { label: "NGINX failover, replica killed", value: "20/20 HTTP 200", evidence: "measured" },
    { label: "my commits on the submission branch", value: "48 / 97", evidence: "implemented" },
  ],
  decisions: [
    {
      id: "saga",
      title: "Choreographed saga rather than orchestrated",
      context: "Phase 1 coordinated line activation through synchronous REST calls: one slow service blocked the whole chain.",
      decision: "Each service reacts to the others' events and publishes its compensation on failure, with no central orchestrator.",
      rationale: "Temporal decoupling: the order is accepted with a 202 even if the HLR is slow, and no service needs to know the full graph.",
      tradeoff: "The flow is no longer readable in one place — hence distributed tracing and a documented event catalogue, without which debugging becomes impractical.",
    },
    {
      id: "outbox",
      title: "Transactional Outbox + skip_locked row locking",
      ...
      decision: "The event is written to the outbox table in the same transaction as the business state; a relay publishes it afterwards, reading its batch with with_for_update(skip_locked=True) and committing at the end of the batch.",
      rationale: "State/event atomicity, and two relays can progress on disjoint batches — which makes replicating the service safe.",
      tradeoff: "At-least-once delivery: consumers must be idempotent. Measurement did in fact surface 1,467 duplicates before the locking fix.",
    },
    {
      id: "acl",
      ...
      decision: "All translation happens in infrastructure adapters; the domain only knows its own concepts.",
      rationale: "A change on the Hub's side — like the 5 real statuses instead of the 3 announced — is fixed in an adapter, not in business rules.",
    ...
    },
    {
      ...
      decision: "import-linter forbids any technical import inside domain/, and CI fails if the rule is crossed.",
      rationale: "The constraint becomes structural rather than declarative — the only kind of discipline that survives pressure.",
  ...
  limits: [
    ...
    "The brief's target (≥ 1,000 ops/s) is not met: 98.7 HTTP req/s, i.e. ~1.5 complete sagas per second under 100 VUs. The gap is localized and quantified, not explained away afterwards.",
    "The saga convergence threshold (95% under 30s) is not met under 100 VUs: convergence is functional but not sized for that load with a synchronous HLR.",
    "Team project: 48 of the 97 commits on the submission branch are mine.",
    "Private repository (GitHub Classroom) — the code can't be published; the numbers above come from the measurement report of the 2026-08-06 campaign.",
  ],
  links: [],
};
